...
웹사이트 우측 패널 옆(모바일에서는 패널 상단)의 언어 전환 탭(`한` / `ES`)을 클릭하여 한국어와 스페인어 버전을 전환할 수 있습니다.

- **한국어 (한)**: 기본 언어로 전체 웹사이트 인터페이스, 카테고리 필터 및 업체 정보가 한국어로 표시됩니다.
- **스페인어 (ES)**: 제목("Pequeño Seúl"), 검색창, 카테고리 필터, 상세 정보 라벨 및 설명이 스페인어로 전환됩니다.
- **자동 저장**: 선택한 언어 설정은 브라우저에 자동 저장되어 재방문 시에도 유지됩니다.
